...
const Select = ({ targetCurrency, chosenCurrency, setTargetCurrency, setConvertedAmount }) => {
            ...
            <DropDownContainer>
                ...
                {isOpen && (
                    <DropDownListContainer>
                        <DropDownList>
                            {QUOTES_DATA.filter(({ id }) => id !== chosenCurrency.id)
                                .map((currency) => {
                                    const { id, img } = currency;
                                    return (
                                        <ListItem key={id} onClick={() => handleCurrencyClick(currency)}>
                                            <CurrencyImage src={img} />
                                            <CurrencyName>{id}</CurrencyName>
                                        </ListItem>
                                    )
